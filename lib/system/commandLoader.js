...
async function loadCommandsAndPlugins(dir = commandsFolder) {
  ...
  for (const fileOrFolder of items) {
    ...
    if (err) {
      console.error(
        chalk.red(`❌ Error de sintaxis en ${fileOrFolder}:\n${format(err)}`)
      )
      continue
    }
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ Error en el plugin ${fileOrFolder}:`),
        e
      )
    }
  }
...
globalThis.reload = async (_ev, filename) => {
  ...
  try {
    ...
    global.plugins[filename.replace(".js", "")] =
    ...
    console.error(
      chalk.red(`❌ Error al recargar ${filename}:\n`),
      e
    )
  }
}
